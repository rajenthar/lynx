...
/**
 * Same verification logic every other Lynx service's {@code JwtAuthFilter}
 * runs — but registered (see {@code WebConfig}) for exactly ONE path,
 * {@code /auth/me}, since every other endpoint here is public by design or
 * authenticates its caller with its own explicit logic (register/login/OTP/
 * {@code TokenController}'s Basic-auth decode).
 */
