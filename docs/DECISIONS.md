...
| `api-gateway` | Built, tested (9 tests) — the one public entry point (other-docs/14): path-based routing to only `auth-service` (`register`/`login`), `account-service` (its `/v1/accounts/**` end-user surface only, never `/internal/accounts/**`), and `transaction-service` (the whole service); per-user/per-IP rate limiting via Spring Cloud Gateway's `RequestRateLimiter` + `RedisRateLimiter` (a token-bucket, same Redis dependency `lynx-idempotency` already uses); the one reactive/WebFlux service in the project, deliberately isolated; never re-verifies JWTs — that stays each downstream service's own job | [other-docs/14](other-docs/14-api-gateway-design-decisions.md) |
| everything else in `services/*` (10 more placeholder dirs) | Not started | — |
...
|---|---|---|
| ~~No API-level rate limiting (per-user/per-IP throttling) anywhere in the system~~ **RESOLVED** — `api-gateway` built and tested (other-docs/14): Spring Cloud Gateway's `RequestRateLimiter` + `RedisRateLimiter`, keyed per-user (JWT `sub`) falling back to per-IP | `api-gateway` (built) | [ADR-004](ADR-004-idempotency.md)'s Security Considerations |
